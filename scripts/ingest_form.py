...
COLUMN_NAMES = [
    ...
    *[
    ...
    "song_ranking",
    "coldplay_liking",
    "extra_1",  # This column is not needed, but we include it to ensure the correct number of columns
    "extra_2"  # This column is not needed, but we include it to ensure the correct number of columns
]
...
def read_form_responses(file_path: str | Path) -> pl.DataFrame:
    ...
    df = df.select([c(col) for col in COLUMN_NAMES[:-2]])  # Select only the columns we need
    return df

def test_read_csv() -> None:
    """Test the CSV reading function."""
    csv_path = Path(__file__).resolve().parent.parent / "circumplex_hubproj_responses.csv"
